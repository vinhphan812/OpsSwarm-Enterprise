...
def decision_request(run: RunRecord) -> str:
    ...
    if d.options:
        lines += ["### Options"]
        for o in d.options:
            lines += [f"- **{o.id}** — {o.description}", f"  - Risk: `{o.risk.value}`",
                      f"  - Estimated recovery: {o.estimated_recovery or 'unknown'}"]
        lines += ["", f"Recommended: `{d.recommended_option or 'none'}`", "", "Respond with one of:",
                  "`/opsswarm approve <option-id>`", "`/opsswarm investigate <request>`", "`/opsswarm reject`",
                  "`/opsswarm abort`"]
    return "\n".join(lines)
