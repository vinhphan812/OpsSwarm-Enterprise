...
class PolicyEngine:
    def __init__(self, cfg: dict): self.cfg=cfg.get("policy",{})
    ...
    def classify_plan(self, plan: RecoveryPlan) -> tuple[str,str]:
        ...
        if not plan.options: return "DENY", "No remediation options"
        if len(plan.options)>1:
            return "DECISION", "Multiple materially different remediation options"
        opt=plan.options[0]; action=self.action(opt.risk)
        if action=="AUTO": return "AUTO", "Policy allows autonomous execution"
        if action=="HUMAN_APPROVAL": return "APPROVAL", "Policy requires human approval"
