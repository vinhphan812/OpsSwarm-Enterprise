...
def parse_issue(number:int, issue:dict)->IncidentContext:
    ...
    return IncidentContext(issue_number=number,title=title,body=body,service=field("Service"),environment=field("Environment"),severity=sev,
        symptoms=[x.strip(" -") for x in symptoms.splitlines() if x.strip()] or [title],customer_impact=field("Customer impact"),
...
async def synthesize_root_cause(oc,agent,run_id,incident,findings,human_inputs)->RootCauseArtifact:
    return RootCauseArtifact.model_validate(await oc.run_json(agent,f"{run_id}-rca",root_cause_prompt(incident,findings,human_inputs)))

async def make_recovery_plan(oc,agent,run_id,incident,root,human_inputs)->RecoveryPlan:
    return RecoveryPlan.model_validate(await oc.run_json(agent,f"{run_id}-plan",recovery_plan_prompt(incident,root,human_inputs)))

async def execute_recovery(oc,agent,run_id,incident,root,option)->ExecutionResult:
    return ExecutionResult.model_validate(await oc.run_json(agent,f"{run_id}-recover",recovery_prompt(incident,root,option.model_dump_json())))

async def verify_recovery(oc,agent,run_id,incident,execution)->VerificationResult:
    return VerificationResult.model_validate(await oc.run_json(agent,f"{run_id}-verify",verify_prompt(incident,execution.model_dump_json())))

async def make_extra_task(oc,agent,run_id,incident,request)->Task:
    return Task.model_validate(await oc.run_json(agent,f"{run_id}-extra",extra_investigation_prompt(incident,request)))
