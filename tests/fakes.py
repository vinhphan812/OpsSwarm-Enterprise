...
class FakeGitHub:
    def __init__(self,issue): self.issue=issue; self.comments=[]; self.labels=[]; self.closed=[]; self.created=[]; self.permissions={"dev":"maintain","reader":"read"}
    ...
    async def create_issue(self,title,body,labels): self.created.append((title,body,labels)); return {"number":900+len(self.created)}
