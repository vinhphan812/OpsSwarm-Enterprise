...
def investigation_and_rca():
    return [
        {'tasks': [
            {'id': 'T1', 'type': 'OBSERVE', 'objective': 'inspect metrics', 'profile': 'observability-investigator',
             'required_capabilities': ['metrics.read'], 'risk': 'read', 'depends_on': [], 'parallelizable': True,
             'expected_output': 'Finding', 'status': 'PENDING'}]},
...
async def test_free_text_never_approves(tmp_path, cfg, issue):
    responses = investigation_and_rca() + [
        {'options': [
            {'id': 'rollback', 'description': 'rollback', 'profile': 'recovery-responder', 'risk': 'risky_write',
             'estimated_recovery': '3m', 'rationale': 'best', 'capabilities': []}], 'recommended_option': 'rollback',
            'confidence': 0.9, 'requires_business_input': False, 'business_input_question': None},
    ]
    gh = FakeGitHub(issue);
    oc = FakeOpenClaw(responses);
    eng = Orchestrator(cfg, gh, oc, str(tmp_path))
...
async def test_readonly_user_cannot_approve(tmp_path, cfg, issue):
    ...
    eng = Orchestrator(cfg, gh, oc, str(tmp_path));
    r = await eng.start_issue(1)
    with pytest.raises(PermissionError): await eng.handle_comment(1, 'reader', '/opsswarm approve rollback', 'read',
                                                                  parse_command('/opsswarm approve rollback'))
